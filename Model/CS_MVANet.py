...
class MultiScaleConv(nn.Module):
    def __init__(self, in_channels, out_channels, sampling_rate):
        ...
        kernel_size1 = (1, int(sampling_rate * 0.5) - 1) # 63
...
class CS_MVANet(nn.Module):
    def __init__(self, eeg_channels, in_channels, emb_size, num_heads, L, W, H, num_classes=2):
        super().__init__()

        self.ms_conv = MultiScaleConv(eeg_channels, emb_size, 128)
        self.cs_attention = CrossScaleAttention(emb_size, num_heads, 0.5)
        self.ffn = FeedForward(emb_size * 3, 0.5)
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.fc_cst = nn.Linear(emb_size * 3, 64)

        self.space_time_stream = SpaceTimeStream(in_channels, L, W, H)
        self.space_freq_stream = SpaceFrequencyStream(in_channels, L, W, H)
        self.fc_mva = nn.Linear(512*2, 64)
        self.bn_mva = nn.BatchNorm1d(64)
    ...
    def forward(self, X_eeg, X_time, X_freq):

        ms1, ms2, ms3 = self.ms_conv(X_eeg)
        att_output = self.cs_attention(ms1, ms2, ms3)
        output = self.ffn(att_output)
        output = output.permute(0, 2, 1)
        x = self.pool(output).squeeze(-1)
        CST_features = self.fc_cst(x)

        x_time = self.space_time_stream(X_time)
        x_freq = self.space_freq_stream(X_freq)
        x = torch.cat((x_time, x_freq), dim=1)
        x = self.fc_mva(x)
        MVA_features = self.bn_mva(x)

        fusion = torch.cat((CST_features, MVA_features), dim=1)
        x = self.fusion_block(fusion)
        x = self.dropout(x)
        x = self.fc_out(x)
        return x